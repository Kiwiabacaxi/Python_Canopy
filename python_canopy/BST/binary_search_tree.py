...
class BinarySearchTree:
    ...
    def insert(self, key: Any, data: Any) -> None:
        """ Insert a node into the tree.
            If the key already exists, raise a DuplicateKeyError.
        """
    ...
    def create_from_list(self, keys: List[Any]) -> None:
        """Create a binary search tree from a list of keys.
        Parameters
        ----------
        keys: `List[Any]`
            A list of keys.
        """
        for key in keys:
            self.insert(key=key, data=None)

    def create_from_sorted_list(self, keys: List[Any]) -> None:
        """Create a balanced binary search tree from a sorted list of keys.
        Parameters
        ----------
        keys: `List[Any]`
            A sorted list of keys.
        """
        self._create_from_sorted_list(keys=keys, start=0, end=len(keys) - 1)

    def _create_from_sorted_list(self, keys: List[Any], start: int,
                                 end: int) -> None:
        """Create a balanced binary search tree from a sorted list of keys.
        Parameters
        ----------
        ...
        if start > end:
            return
        mid = (start + end) // 2
        self.insert(key=keys[mid], data=None)
        self._create_from_sorted_list(keys=keys, start=start, end=mid - 1)
        self._create_from_sorted_list(keys=keys, start=mid + 1, end=end)

    def print_in_order(self) -> None:
        """Print the tree in order.
        """
        self._print_in_order(node=self.root)

    def _print_in_order(self, node: Optional[Node]) -> None:
        """Print the tree in order.
        """
        if node is not None:
            self._print_in_order(node=node.left)
            print(node.key)
            self._print_in_order(node=node.right)
...
def basic_tree() -> list:
    """ Returns a basic tree structure as a list of lists.
        For building a tree
    """
    l1 = []
