...
class AVLTree:
    ...
    def _insert(self, key, cur_node):
        if key < cur_node.key:
            if cur_node.left == None:                  # se o filho esquerdo for vazio
                cur_node.left = node(key)              # cria o nó
                cur_node.left.parent = cur_node    # define o pai do nó
            ...
            if cur_node.right == None:                 # se o filho direito for vazio
                cur_node.right = node(key)             # cria o nó
                cur_node.right.parent = cur_node   # define o pai do nó
                # verifica se a árvore está balanceada
    ...
    def _height(self, cur_node, current_height):
        if cur_node == None:  # empty node // se o nó for vazio
